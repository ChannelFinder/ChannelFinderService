...
public class ElasticConfigIT {
    ...
    static void teardown(ElasticConfig elasticConfig) throws IOException {
        ...
        String[] indexes = new String[] {elasticConfig.getES_CHANNEL_INDEX(), elasticConfig.getES_PROPERTY_INDEX(), elasticConfig.getES_TAG_INDEX()};
    ...
    /**
     * Makes sure the indexes are recreated before running a test
     * This can happen if two tests share the same ElasticConfig bean and teardown is run at the end.
     * For an example see PropertyManagerIT and PropertyValidationIT use the same @WebMvcTest annotation
     * and then share the ElasticConfig Bean during test runs.
     *
     * @param elasticConfig Bean with configuration
     */
