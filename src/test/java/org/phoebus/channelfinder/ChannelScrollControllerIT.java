...
class ChannelScrollControllerIT {
  ...
  void searchNameTest() throws InterruptedException {
    ...
    List<ChannelDto> result = new ArrayList<>(scrollResult.channels());
    while (scrollResult.channels().size() == 100) {
      scrollResult = channelScroll.query(scrollResult.id(), searchParameters);
      result.addAll(scrollResult.channels());
    }
    Assertions.assertTrue(result.size() == 1 && result.get(0).name().equals(channelNames.get(0)));
    ...
    result = new ArrayList<>(scrollResult.channels());
    while (scrollResult.channels().size() == 100) {
      scrollResult = channelScroll.query(scrollResult.id(), searchParameters);
      result.addAll(scrollResult.channels());
    }
