...
public class NTXmlUtil {
  /**
   * A helper method to convert the result of the channelfinder v4 service to a list of {@link
   * Channel}
   *
  ...
  public static synchronized List<Channel> parse(PVAStructure result) {
    ...
    if (names.contains(COLUMN_OWNER)) {
      ...
      for (int i = 0; i < channels.size(); i++) {
        if (owners.get(i).isPresent()) channels.get(i).setOwner(owners.get(i).get());
      }
    }
