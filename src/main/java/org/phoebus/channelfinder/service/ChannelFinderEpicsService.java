 ...
 * <p>Request:
 *
 * <p>The client requests a query as a NTURI pvStructure.
 *
 * <p>Result:
 *
 * <p>The service returns the result as an NTTable pvStructure.
 *
 * @author Kunal Shroff
 */
@Service
                  ...
                  .forEach(
                      prop -> {
                        if (!channelPropertyTable.containsKey(prop.getName())) {
                          channelPropertyTable.put(
                              prop.getName(), Arrays.asList(new String[result.size()]));
                        }
                        channelPropertyTable.get(prop.getName()).set(index, prop.getValue());
                      });
            });
        PVATable.PVATableBuilder ntTableBuilder =
            PVATable.PVATableBuilder.aPVATable().name(SERVICE_DESC);

        channelTable
            .keySet()
            .forEach(
                name ->
                    ntTableBuilder.addColumn(
                        new PVAStringArray(name, channelTable.get(name).toArray(String[]::new))));
        channelPropertyTable
            ...
            .forEach(
                name ->
                    ntTableBuilder.addColumn(
                        new PVAStringArray(
                            name, channelPropertyTable.get(name).toArray(String[]::new))));
        channelTagTable
            .keySet()
            .forEach(
                name ->
