...
public class ChannelRepository implements CrudRepository<XmlChannel, String> {
     ...
     */
    public <S extends XmlChannel> S index(XmlChannel property) {
    ...
    public <S extends XmlChannel> Iterable<S> indexAll(List<XmlChannel> properties) {
        RestHighLevelClient client = esService.getIndexClient();
        try {
            BulkRequest bulkRequest = new BulkRequest();
            for (XmlChannel property : properties) {
                IndexRequest indexRequest = new IndexRequest(ES_CHANNEL_INDEX, ES_CHANNEL_TYPE)
                        .id(property.getName())
                        .source(objectMapper.writeValueAsBytes(property), XContentType.JSON);
    ...
    public Optional<XmlChannel> findById(String id, boolean withChannels) {
        ...
        try {
            ...
            if (response.isExists()) {
                XmlChannel property = objectMapper.readValue(response.getSourceAsBytesRef().streamInput(),
                        XmlChannel.class);
